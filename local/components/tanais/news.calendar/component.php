...
define("EVENT_TYPE_HL_BLOCK_ID", 1);
...
if(!isset($arParams["DATA_PLACEMENT"]))
	$arParams["DATA_PLACEMENT"] = "top";
...
if($this->StartResultCache(false, array($currentMonth, $currentYear, $todayYear, $todayMonth, $todayDay)))
{
	...
	$arSelectedFields = Array("ACTIVE", $arParams["DATE_FIELD"], "ID", "IBLOCK_ID", "SITE_ID", "DETAIL_PAGE_URL", "NAME", "LANG_DIR", "SORT", "IBLOCK_TYPE", "PREVIEW_TEXT", "PREVIEW_TEXT_TYPE", "PROPERTY_TYPE");
		...
		for ($j = 0; $j < 7; $j++)
		{
			...
			if(is_set($arDays[$tmpDate]))
			{
				...
				foreach($arDays[$tmpDate] as $dayNews)
				{
					...
					if($dayNews["PROPERTY_TYPE_VALUE_ID"] > 0) {
						$hlblock = HL\HighloadBlockTable::getById(EVENT_TYPE_HL_BLOCK_ID)->fetch(); 
						...
						$rsData = $entity_data_class::getList(array(
						   "select" => array("*"),
						   "filter" => array("UF_XML_ID" => $dayNews["PROPERTY_TYPE_VALUE"])
						));

						if($arData = $rsData->Fetch()) {
							
							if($arData["UF_ICON"] > 0)
								$eventIcon = CFile::GetPath($arData["UF_ICON"]);
						}
