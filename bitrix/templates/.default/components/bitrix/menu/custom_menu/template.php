		...
		<table>
			<tr>
				<?foreach($arResult as $arItem):?>
					...
					$arItem['bManyItemsMenu'] = (isset($arItem['PARAMS']['CLASS']) && strpos($arItem['PARAMS']['CLASS'], 'wide_menu') !== false);
					?>
					<td class="menu-item unvisible <?=($arItem["CHILD"] ? "dropdown" : "")?> <?=($bWideMenu ? '' : '');?> <?=(isset($arItem["PARAMS"]["CLASS"]) ? $arItem["PARAMS"]["CLASS"] : "");?>  <?=($arItem["SELECTED"] ? "active" : "")?>">
						<div class="wrap">
							...
							<?if($bShowChilds):?><!-- bShowChilds --><?endif;?>
							<?if($arItem["CHILD"] && $bShowChilds):?>
								<?$bRightSideShow = ($arItem['PARAMS']['CLASS'] || $arItem['PARAMS']['BRANDS']) && $bRightSide;?>
								
								<?if($bWideMenu):?><!-- bWideMenu --><?endif;?>
								<?if($bRightSideShow):?><!-- bRightSideShow --><?endif;?>
								<?if($arItem["bManyItemsMenu"]):?><!-- bManyItemsMenu --><?endif;?>
								<!-- <? //print_r($arItem); ?> -->
								<span class="tail"></span>
								<div class="dropdown-menu <?=$bRightSide ? 'with_right_block '.$RightContent : ''?> <?= $arItem['bManyItemsMenu'] ? 'long-menu-items' : '' ?>" >
									...
									<div class="customScrollbar">
										...
										<?if($bRightSideShow && !$arItem['bManyItemsMenu']):?>
										...
										<ul class="menu-wrapper <?='menu-type-'.$MENU_TYPE?>">
											<?foreach($arItem["CHILD"] as $arSubItem):?>
												...
												<?if($MENU_TYPE == 2 && !$arItem['bManyItemsMenu']):?>
												...
												<?else: // type 1?>
													...
													$bIcon = (isset($arSubItem['PARAMS']['SECTION_ICON'])) && $arSubItem['PARAMS']['SECTION_ICON'];
													?>
													<li class="<?=($arSubItem["CHILD"] && $bShowChilds ? "dropdown-submenu" : "")?> <?=$bIcon ? 'icon' : ''?> <?=($arSubItem["SELECTED"] ? "active" : "")?> <?=($bHasPicture ? "has_img" : "")?> <?= $arItem['bManyItemsMenu'] ? 'parent-items' : '' ?>">
														<?if($arItem['bManyItemsMenu']):?>
