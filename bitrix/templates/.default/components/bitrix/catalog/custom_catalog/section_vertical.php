...
<?
$arAvailableCounts = array(5, 10, 20, 50);

$pageElementCount = isset($_COOKIE['showBy']) && intval(
    $_COOKIE['showBy']
) ? $_COOKIE['showBy'] : 5; //$arParams['PAGE_ELEMENT_COUNT'];
...
<div class="cat_items">
    ...
    $CHECK_PROPERTY = array('CITY' => $user_city);
    ?>
    <?
    if ($isFilter || $isSidebar): ?>
        <div class="title_main_mob"><? $APPLICATION->ShowTitle(false)?></div>
        <div class="button_filter_mob"><span>Фильтры</span></div>
        <div class="col-md-3 catalog_col filter col-sm-3<?= (isset($arParams['FILTER_HIDE_ON_MOBILE']) && $arParams['FILTER_HIDE_ON_MOBILE'] === 'Y' ? ' hidden-xs' : '') ?>">
            <?
            if ($isFilter): ?>
...
//                            "FILTER_NAME" => $arParams["FILTER_NAME"],
    ...
    <div class="<?= (($isFilter || $isSidebar) ? "col-md-9 catalog_col col-sm-9 " : "col-xs-12") ?>">
        <span id="AJAX_ID" data-id="<?= $arParams['AJAX_ID']; ?>"></span>
        <?
        $new_arrFilter = array();
        foreach ($GLOBALS['arrFilter'] as $key => $value):
            $new_key = str_replace('=PROPERTY_', '', $key);
            array_push($new_arrFilter, [$new_key => $value]);
        endforeach;
        $count_element = CIBlockSection::GetSectionElementsCount(
            $arResult["VARIABLES"]["SECTION_ID"],
            //Array ('1193' =>  Array (0 => 8639 ))
            array('PROPERTY' => $new_arrFilter[0])
        ); ?>
            ...
            <div class="text-right">
                <select name="productType" id="productType">
                    ...
                    foreach ($arAvailableCounts as $value): ?>
                        <option value="<?= $APPLICATION->GetCurPageParam('showBy=' . $value, array('showBy')
                        ); ?>" <?= ($value == $pageElementCount ? 'selected' : ''); ?>>На странице: <?= $value; ?> шт.
                        </option>
                        ...
                        <div data-entity="parent-container">
                            ...
                            if (!isset($arParams['GIFTS_SECTION_LIST_HIDE_BLOCK_TITLE']) || $arParams['GIFTS_SECTION_LIST_HIDE_BLOCK_TITLE'] !== 'Y') {
                                ...
                                <div class="catalog-block-header" data-entity="header" data-showed="false"
                                    ...
                                    <?= ($arParams['GIFTS_SECTION_LIST_BLOCK_TITLE'] ?: \Bitrix\Main\Localization\Loc::getMessage(
                                        'CT_GIFTS_SECTION_LIST_BLOCK_TITLE_DEFAULT'
                                    )) ?>
                                </div>
                                <?
                            }
