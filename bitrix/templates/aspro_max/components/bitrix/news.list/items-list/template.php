...
$documentsMode = $documentsMode || in_array('DOCUMENT', $arParams['PROPERTY_CODE']);
?>
								...
								<?foreach($arSection['ITEMS'] as $i => $arItem):?>
									...
									<?ob_start();?>
										...
										<?if(strlen($arItem['FIELDS']['NAME'])):?>
											...
											<?if($documentsMode):?>
												<?if(!empty($arItem['PROPERTIES']['URL_WITH_3D']['VALUE'])):?>
													<div class="title">
														<a href="<?=$arItem['PROPERTIES']['URL_WITH_3D']['VALUE']?>" class="dark-color" target="_blank">
															<?=$arItem['NAME']?>
														</a>
													</div>
												<? else: ?>
													<div class="title">
														...
														<div class="size muted font_xs"><?=$docFileSize?$docFileSize:$fileSize;?></div>
													</div>
												<? endif; ?>
											<?else:?>
												<div class="title <?=($licensesMode && $arParams['VIEW_TYPE'] == 'table' ? '' : 'font_mlg')?> ">
													...
													<?if($licensesMode && $fileSize):?>
														<?if($arParams['VIEW_TYPE'] == 'table'):?>
															<div class="size muted font_xs"><?=$docFileSize?$docFileSize:$fileSize;?></div>
														<?else:?>
															<span class="size muted font_xs"><?=$docFileSize?$docFileSize:$fileSize;?></span>
														<?endif;?>
	                                                <?endif;?>
												</div>
											<?endif;?>
											...
											<div class="item colored_theme_hover_bg-block <?=($bImage ? '' : ' wti')?> box-shadow bordered clearfix " id="<?=$this->GetEditAreaId($arItem['ID'])?>">
												...
												<div class="body-info">
													...
													<?if($documentsMode && $arDocFile):?>
														<?if(!empty($arItem['PROPERTIES']['URL_WITH_3D']['VALUE'])):?>
															<a href="<?=$arItem['PROPERTIES']['URL_WITH_3D']['VALUE']?>" class="colored_theme_hover_bg-el bordered dark-color rounded3 muted download_wrap" target="_blank">
