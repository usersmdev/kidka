...
<?if($arResult["ITEMS"]):?>
	...
	<?endif;?>
		...
		<?foreach($arResult["ITEMS"] as $arItem){?>
			...
			<?endif;?>
			<div class="col-lg-<?=$col;?><?if($bBigBlock):?> col-md-8 col-sm-12 col-xs-12<?else:?> col-md-4 col-sm-6 col-xs-6<?endif;?> col-xxs-12 item item-parent item_block<?=($bBigBlock ? ' big' : '');?>">
				...
				<div class="catalog_item_wrapp catalog_item item_wrap main_item_wrapper<?=($bBigBlock ? ' big' : '');?> <?=($arItem["OFFERS"] ? 'has-sku' : '')?>" id="<?=$arItem["strMainID"]?>">
					<div class="inner_wrap <?=$arParams["TYPE_VIEW_BASKET_BTN"]?>">
						...
						<div class="item_info">
							...
							<div class="sa_block" data-fields='<?=Json::encode($arParams["FIELDS"])?>' data-user-fields='<?=Json::encode($arParams["USER_FIELDS"])?>'>
								<?=$arQuantityData["HTML"];?>
