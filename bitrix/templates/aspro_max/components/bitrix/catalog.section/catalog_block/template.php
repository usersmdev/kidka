		...
		<?foreach($arResult["ITEMS"] as $arItem){?>
			...
			<?ob_start();?>
				<div class="footer_button <?=($arItem["OFFERS"] && $arItem['OFFERS_PROP'] ? 'has_offer_prop' : '');?> inner_content js_offers__<?=$arItem['ID'];?>_<?=$arParams["FILTER_HIT_PROP"]?><?=($arParams["TYPE_VIEW_BASKET_BTN"] == "TYPE_2" ? ' n-btn' : '')?>">
					...
					<? // ext ?>
					<div class="wrapp_one_click wrapp_one_click_text">
						<?if(/*$arAddToBasketData["ACTION"] == "ADD" &&*/$arItem["OFFERS_MORE"] != "Y" && $arAddToBasketData["CAN_BUY"]):?>
								<?if($currentSKUID && $currentSKUIBlock):?>
									<span class="btn btn-transparent-border-color  round-ignore  type_block transition_bg one_click has-ripple" data-item="<?=$currentSKUID?>" data-iblockID="<?=$currentSKUIBlock?>" data-quantity="<?=$arAddToBasketData["MIN_QUANTITY_BUY"];?>" onclick="oneClickBuy('<?=$currentSKUID?>', '<?=$currentSKUIBlock?>', this)" title="<?=Loc::getMessage('ONE_CLICK_BUY')?>">
								<?else:?>
									<span class="btn btn-transparent-border-color  round-ignore  type_block transition_bg one_click has-ripple" data-item="<?=$arItem["ID"]?>" data-iblockID="<?=$arItem["IBLOCK_ID"]?>" data-quantity="<?=$arAddToBasketData["MIN_QUANTITY_BUY"];?>" onclick="oneClickBuy('<?=$arItem["ID"]?>', '<?=$arItem["IBLOCK_ID"]?>', this)" title="<?=Loc::getMessage('ONE_CLICK_BUY')?>">
								<?endif;?>
									Купить в 1 клик
								</span>
						<?endif;?>
