;(function() {
	...
	BX.Sale.BasketComponent = {
		...
		ids: {
			...
			total2: 'basket-total-montage-price',
			total3: 'basket-total-design-price',
			...
			itemListTable2: 'basket-item-montage-table',
		...
		fillTotalBlocks: function()
		{
			...
			var totalNodes2 = this.getEntities(this.getCacheNode(this.ids.basketRoot), 'basket-total-montage-block');

			if (totalNodes2 && totalNodes2.length) {
				var totalTemplate2 = this.getTemplate('basket-total-montage-template');
				if (totalTemplate2) {
					var totalRender2 = this.render(totalTemplate2, this.result.TOTAL_RENDER_DATA);

					for (var i in totalNodes) {
						if (totalNodes2.hasOwnProperty(i) && BX.type.isDomNode(totalNodes2[i])) {
							totalNodes2[i].innerHTML = totalRender2;
						}
					}
				}
			}
		...
		checkTotalAnimation: function(totalData)
		{
			if (this.result && this.result.TOTAL_RENDER_DATA && parseFloat(this.result.TOTAL_RENDER_DATA.PRICE) > parseFloat(totalData.PRICE))
			{
				...
				totalData.MONTAGE_PRICE_NEW = totalData.MONTAGE_PRICE;
				totalData.MONTAGE_PRICE = this.result.TOTAL_RENDER_DATA.MONTAGE_PRICE;

				totalData.MONTAGE_PRICE_FORMATED_NEW = totalData.MONTAGE_PRICE_FORMATED;
				totalData.MONTAGE_PRICE_FORMATED = this.result.TOTAL_RENDER_DATA.MONTAGE_PRICE_FORMATED;

				totalData.DESIGN_PRICE_NEW = totalData.DESIGN_PRICE;
				totalData.DESIGN_PRICE = this.result.TOTAL_RENDER_DATA.DESIGN_PRICE;

				totalData.DESIGN_PRICE_FORMATED_NEW = totalData.DESIGN_PRICE_FORMATED;
				totalData.DESIGN_PRICE_FORMATED = this.result.TOTAL_RENDER_DATA.DESIGN_PRICE_FORMATED;

				this.addPriceAnimationData(this.ids.total, totalData.PRICE, totalData.PRICE_NEW, totalData.CURRENCY);
				this.addPriceAnimationData(this.ids.total2, totalData.MONTAGE_PRICE, totalData.MONTAGE_PRICE_NEW, totalData.CURRENCY);
				this.addPriceAnimationData(this.ids.total3, totalData.DESIGN_PRICE, totalData.DESIGN_PRICE_NEW, totalData.CURRENCY);
			}
		...
		createBasketItem: function(itemId)
		{
			...
			// var basketItemTemplate2 = this.getTemplate('basket-item-montage-template');
			if (basketItemTemplate)
			{
				var basketItemHtml = this.renderBasketItem(basketItemTemplate, this.items[itemId]);
				// var basketItemHtml2 = this.renderBasketItem(basketItemTemplate2, this.items[itemId]);
				...
				if (this.shownItems.length && sortIndex >= 0)
				{
					if (sortIndex < BX.util.array_search(this.shownItems[0], this.sortedItems))
					{
						...
						// BX(this.ids.item2 + this.shownItems[0]).insertAdjacentHTML('beforebegin', basketItemHtml2);
					...
					else if (sortIndex > BX.util.array_search(this.shownItems[this.shownItems.length - 1], this.sortedItems))
					{
						...
						// BX(this.ids.item2 + this.shownItems[this.shownItems.length - 1]).insertAdjacentHTML('afterend', basketItemHtml2);
					...
					else
					{
						...
						// BX(this.ids.item2 + this.sortedItems[sortIndex + 1]).insertAdjacentHTML('beforebegin', basketItemHtml2);
				...
				else
				{
					...
					// this.getCacheNode(this.ids.itemListTable2).insertAdjacentHTML('beforeend', basketItemHtml2);
					this.shownItems.push(itemId);
				}
					...
					for (var nodeId in animationData.start)
					{
						if (animationData.start.hasOwnProperty(nodeId))
						{
							if (!nodeCache[nodeId])
							{
								...
								else if (nodeId === this.ids.total2)
								{
									nodeCache[nodeId] = this.getEntities(this.getCacheNode(this.ids.basketRoot), this.ids.total2);
								}
								else if (nodeId === this.ids.total3)
								{
									nodeCache[nodeId] = this.getEntities(this.getCacheNode(this.ids.basketRoot), this.ids.total3);
								}
								else
								{
									var node = BX(nodeId);
				...
				complete: BX.delegate(function() {
					...
					for (nodeId in animationData.start)
					{
						if (animationData.start.hasOwnProperty(nodeId))
						{
							...
							else if (nodeId.indexOf(this.ids.total2) !== -1) {
								type = 'TOTAL2';
								itemId = '';
							}
							else if (nodeId.indexOf(this.ids.total3) !== -1) {
								type = 'TOTAL3';
								itemId = '';
							}
							else
							...
							if (BX.type.isNotEmptyString(type))
							{
								...
								else if (type === 'TOTAL')
								{
									...
									this.result.TOTAL_RENDER_DATA.MONTAGE_PRICE = animationData.finish[nodeId];
									delete this.result.TOTAL_RENDER_DATA.MONTAGE_PRICE_NEW;
									this.result.TOTAL_RENDER_DATA.MONTAGE_PRICE_FORMATED = formattedPrice;
									delete this.result.TOTAL_RENDER_DATA.MONTAGE_PRICE_FORMATED_NEW;
								}
								else if (type === 'TOTAL3') {
									this.result.TOTAL_RENDER_DATA.DESIGN_PRICE = animationData.finish[nodeId];
									delete this.result.TOTAL_RENDER_DATA.DESIGN_PRICE_NEW;
									this.result.TOTAL_RENDER_DATA.DESIGN_PRICE_FORMATED = formattedPrice;
									delete this.result.TOTAL_RENDER_DATA.DESIGN_PRICE_FORMATED_NEW;
								}
							}
						}
					}
		...
		deleteBasketItem: function(itemId, restore, final)
		{
			...
			else
			{
				...
				BX.remove(BX(this.ids.item2 + itemId));
			}
		...
		{
			var basketItemNode = BX(this.ids.item + itemId);
			var basketItemNode2 = BX(this.ids.item2 + itemId);
			...
			if (basketItemTemplate)
			{
				...
				if (basketItemNode2) {
					var basketItemHtml2 = this.renderBasketItem(basketItemTemplate2, this.items[itemId]);
					basketItemNode2.insertAdjacentHTML('beforebegin', basketItemHtml2);
					BX.remove(basketItemNode2);
				}
		...
		restoreBasketItems: function(items)
		{
			...
			var oldItemId, newItemId, basketItemNode, basketItemNode2;
			...
			for (oldItemId in items)
			{
				if (items.hasOwnProperty(oldItemId))
				{
					...
					if (this.isItemShown(oldItemId))
					{
						...
						basketItemNode2 = BX(this.ids.item2 + oldItemId);
						if (BX.type.isDomNode(basketItemNode)) {
							basketItemNode2.id = this.ids.item2 + newItemId;
							basketItemNode2.setAttribute('data-id', newItemId);
						}
					}
