...
<?foreach($arBlockOrder as $code):?>
	<?//news?>
		...
		<?if($html && strpos($html, 'error') === false):?>
			<div class="ordered-block <?=$code?> with-title"><!--1-->
	...
	<?elseif($code == 'services' && $templateData['LINK_SERVICES']):?>
		...
		<?$html=ob_get_clean();?>
		<?if($html && strpos($html, 'error') === false):?>
			<div class="ordered-block <?=$code?> with-title"><!--2-->
				<div class="ordered-block__title option-font-bold font_lg">
	...
	<?elseif($code == 'brands' && $templateData['LINK_BRANDS']):?>
		...
		<?$html=ob_get_clean();?>
		<?if($html && strpos($html, 'error') === false):?>
			<div class="ordered-block <?=$code?> brands-block with-title"><!--3-->
				<div class="ordered-block__title option-font-bold font_lg">
		...
		<?ob_start()?>
			...
			$APPLICATION->IncludeComponent(
				...
				array(
					...
					"SIZE_IN_ROW" => ($bHideLeftBlock ? '4' : '4'),
					"TYPE_IMG" => "lg",
					"BORDERED" => "Y",
					"SHOW_SUBSCRIBE" => "Y",
					"TITLE_SUBSCRIBE" => "",
	...
	<?elseif($code == 'vacancy' && $templateData['LINK_VACANCY']):?>
		...
		<?if($html && trim($html) && strpos($html, 'error') === false):?>
			<div class="ordered-block <?=$code?> with-title"><!--4-->
				<div class="ordered-block__title option-font-bold font_lg ">
	...
	<?elseif($code == 'blog' && $templateData['LINK_BLOG']):?>
		...
		<?$html=ob_get_clean();?>
		<?if($html && trim($html) && strpos($html, 'error') === false):?>
			<div class="ordered-block <?=$code?> with-title"><!--5-->
				<div class="ordered-block__title option-font-bold font_lg ">
	...
	<?elseif($code == 'partners' && $templateData['LINK_PARTNERS']):?>
		...
		<?$html=ob_get_clean();?>
		<?if($html && trim($html) && strpos($html, 'error') === false):?>
			<div class="ordered-block <?=$code?> with-title"><!--6-->
				<div class="ordered-block__title option-font-bold font_lg ">
		...
		<?endif;?>
	<?//offers?>
	<?elseif($code == 'offers' && $templateData['LINK_OFFERS']):?>
		<?ob_start();?>
			<?$GLOBALS['arrOffersFilter'] = array('ID' => $templateData['LINK_OFFERS']);?>
			<?$APPLICATION->IncludeComponent(
				"bitrix:news.list",
				"news-list",
				array(
					"IBLOCK_TYPE" => "aspro_max_content",
					"IBLOCK_ID" => $arParams['IBLOCK_LINK_OFFERS_ID'],
					"NEWS_COUNT" => "20",
					"SORT_BY1" => "SORT",
					"SORT_ORDER1" => "ASC",
					"SORT_BY2" => "ID",
					"SORT_ORDER2" => "DESC",
					"FILTER_NAME" => "arrOffersFilter",
					"FIELD_CODE" => array(
						0 => "NAME",
						1 => "DETAIL_PAGE_URL",
						2 => "PREVIEW_TEXT",
		...
		<?endif;?>
	<?//contents?>
	<?elseif($code == 'contents' && $templateData['LINK_CONTENTS']):?>
		<?ob_start();?>
			<?$GLOBALS['arrContentsFilter'] = array('ID' => $templateData['LINK_CONTENTS']);?>
			<?$APPLICATION->IncludeComponent(
				"bitrix:news.list",
				"news-list",
				array(
					"IBLOCK_TYPE" => "aspro_max_content",
					"IBLOCK_ID" => $arParams['IBLOCK_LINK_CONTENTS_ID'],
					"NEWS_COUNT" => "20",
					"SORT_BY1" => "SORT",
					"SORT_ORDER1" => "ASC",
					"SORT_BY2" => "ID",
					"SORT_ORDER2" => "DESC",
					"FILTER_NAME" => "arrContentsFilter",
					"FIELD_CODE" => array(
						0 => "NAME",
						1 => "DETAIL_PAGE_URL",
						2 => "PREVIEW_TEXT",
	...
	<?elseif($code == 'goods'):?>
		<?if((in_array('LINK_GOODS', $arParams['PROPERTY_CODE']) || ($arParams['SHOW_LINKED_PRODUCTS'] == 'Y' && strlen($arParams['LINKED_PRODUCTS_PROPERTY'])))
			...
			<div class="ordered-block <?=$code?> cur with-title"><!--7-->
				<?if($arParams["T_GOODS"]):?>
					<div class="ordered-block__title option-font-bold font_lg">
						<?=$arParams["T_GOODS"];?>
					</div>
	...
	<?elseif($code == 'sale' && $templateData['LINK_SALE']):?>
		...
		<?$html=ob_get_clean();?>
		<?if($html && trim($html) && strpos($html, 'error') === false):?>
			<div class="ordered-block <?=$code?> with-title"><!--8-->
				<div class="ordered-block__title option-font-bold font_lg ">
