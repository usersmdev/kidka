...
$templateData = array(
	...
	// 'ASSOCIATED_EXTRA' => $arResult['ASSOCIATED_EXTRA'],
...
// $templateData['QUANTITY_DATA'] = $arQuantityData = CatalogHelper::GetQuantityArray($totalCount, $arItemIDs["ALL_ITEM_IDS"], ($arParams["USE_STORE"] == "Y" && $arResult["STORES_COUNT"] && $arResult['CATALOG_TYPE'] != CCatalogProduct::TYPE_SET && (!$arResult["OFFERS"] || ($arResult["OFFERS"] && $arParams["TYPE_SKU"]!="N")) ? "Y" : "N"));
			...
			<div class="list-offers ajax_load">
				...
				<div class="table-view flexbox flexbox--row">
					<?foreach($arResult["OFFERS"] as $key => $arSKU):?>
						...
						// $arskuQuantityData = CatalogHelper::getQuantityArray($skutotalCount);
	...
	<div class="product-info <?=(!$showCustomOffer ? "noffer" : "");?> product-info--type2" id="<?=$arItemIDs["strMainID"];?>">
		...
		<div class="flexbox flexbox--row">
							...
							<div id="photo-sku" class="product-detail-gallery__item product-detail-gallery__item--middle text-center">
								<?if($arFirstPhoto["BIG"]["src"]):?>
									<a href="<?=($viewImgType=="POPUP" ? $arFirstPhoto["BIG"]["src"] : "javascript:void(0)");?>" <?=($bIsOneImage ? '' : 'data-fancybox="gallery"')?> class="product-detail-gallery__link <?=($viewImgType=="POPUP" ? "popup_link fancy" : "line_link");?>" title="<?=$title;?>">
										<img id="<? echo $arItemIDs["ALL_ITEM_IDS"]['PICT']; ?>" class="lazy_10 product-detail-gallery__picture <?=($viewImgType=="MAGNIFIER" ? "zoom_picture" : "");?>" data-src="<?=($arFirstPhoto["SMALL"]["src"] ? $arFirstPhoto["SMALL"]["src"] : $arFirstPhoto["SRC"])?>" src="<?=\Aspro\Functions\CAsproMax::showBlankImg(($arFirstPhoto["SMALL"]["src"] ? $arFirstPhoto["SMALL"]["src"] : $arFirstPhoto["SRC"]))?>" <?=($viewImgType=="MAGNIFIER" ? 'data-xoriginal="'.$arFirstPhoto["BIG"]["src"].'" data-xpreview="'.$arFirstPhoto["THUMB"]["src"].'"' : "");?> alt="<?=$alt;?>" title="<?=$title;?>"<?//=(!$i ? ' itemprop="image"' : '')?>/>
									</a>
								<?else:?>
									<img id="<? echo $arItemIDs["ALL_ITEM_IDS"]['PICT']; ?>" class="lazy_9 product-detail-gallery__picture one" data-src="<?=\Aspro\Functions\CAsproMax::showBlankImg($arFirstPhoto["SRC"])?>" src="<?=$arFirstPhoto["SRC"]?>" alt="<?=$alt;?>" title="<?=$title;?>" />
								<?endif;?>
							</div>
						<?}else{
								...
								<div id="photo-sku" class="product-detail-gallery__item product-detail-gallery__item--big text-center">
									<?if($arFirstPhoto["BIG"]["src"]):?>
										<a href="<?=$arFirstPhoto["BIG"]["src"];?>" data-fancybox="gallery" class="product-detail-gallery__link popup_link fancy" title="<?=$title;?>">
											<img id="<? echo $arItemIDs["ALL_ITEM_IDS"]['PICT']; ?>" class="lazy_6 product-detail-gallery__picture" data-src="<?=$arFirstPhoto["SMALL"]["src"]?>" src="<?=\Aspro\Functions\CAsproMax::showBlankImg($arFirstPhoto["SMALL"]["src"])?>" alt="<?=$alt;?>" title="<?=$title;?>"<?//=(!$i ? ' itemprop="image"' : '')?>/>
										</a>
									<?else:?>
										<img id="<? echo $arItemIDs["ALL_ITEM_IDS"]['PICT']; ?>" class="lazy_5 product-detail-gallery__picture" data-src="<?=\Aspro\Functions\CAsproMax::showBlankImg($arFirstPhoto["SRC"])?>" src="<?=$arFirstPhoto["SRC"]?>" alt="<?=$alt;?>" title="<?=$title;?>" />
									<?endif;?>
								</div>
							<?}else{
								if($arResult["MORE_PHOTO"]){?>
										...
										<div id="photo-<?=$i?>" class="product-detail-gallery__item product-detail-gallery__item--big text-center">
											<?if(!$isEmpty){?>
												<a href="<?=$arImage["BIG"]["src"];?>" data-fancybox="gallery" class="product-detail-gallery__link popup_link fancy" title="<?=$title;?>">
													<img class="lazy_4 product-detail-gallery__picture" data-src="<?=$arImage["SMALL"]["src"]?>" src="<?=\Aspro\Functions\CAsproMax::showBlankImg($arImage["SMALL"]["src"])?>" alt="<?=$alt;?>" title="<?=$title;?>"<?//=(!$i ? ' itemprop="image"' : '')?>/>
												</a>
											<?}else{?>
												<img class="lazy_3 product-detail-gallery__picture" data-src="<?=\Aspro\Functions\CAsproMax::showBlankImg($arImage["SRC"])?>" src="<?=$arImage["SRC"]?>" alt="<?=$alt;?>" title="<?=$title;?>" />
											<?}?>
										</div>
									<?}?>
								<?}
										...
										<?if($arResult["MORE_PHOTO"]):?>
											<?foreach($arResult["MORE_PHOTO"] as $i => $arImage):?>
												<div id="photo-<?=$i?>" class="product-detail-gallery__item text-center  product-detail-gallery__item--thmb" data-big="<?=$arImage["BIG"]["src"];?>">
													<?if(!$isEmpty){?>
														<img class="lazy_2 product-detail-gallery__picture" data-src="<?=$arImage["SMALL"]["src"]?>" src="<?=\Aspro\Functions\CAsproMax::showBlankImg($arImage["SMALL"]["src"])?>" alt="<?=$alt;?>" title="<?=$title;?>"/>
													<?}else{?>
														<img class="lazy_1 product-detail-gallery__picture" data-src="<?=\Aspro\Functions\CAsproMax::showBlankImg($arImage["SRC"])?>" src="<?=$arImage["SRC"]?>" alt="<?=$alt;?>" title="<?=$title;?>" />
													<?}?>
												</div>
											<?endforeach;?>
										<?endif;?>
			...
			<div class="product-main">
				...
				<div class=" <?=!$bShowPropsBlock ? 'justify-center' : ''?>">
					<?if($bShowPropsBlock):?>
						<div class="product-chars">
												...
												<?if($j<$arParams['VISIBLE_PROP_COUNT']):?>
													<div class="properties__item properties__item--compact font_xs">
														<div class="properties__title muted properties__item--inline">
															<?
															$propNameCode = preg_match('/\d+$/', $arProp['CODE'], $matches)
																? preg_replace('/(\d+)$/', '_NAME$1', $arProp['CODE'])
																: '';
															$propName = $propNameCode
																? $arResult['PROPERTIES'][$propNameCode]['VALUE']
																: '';
															if ($propNameCode && !$propName) {
																$propName = preg_replace('/\s+\d+$/', '', $arProp['NAME']);
															}
															?>
															<?if($propName):?>
																<?=$propName?>
							...
							?>
								<?$iTab = 0;?>
								<?foreach($arSmallTabOrder as $value):?>
									<?//props?>
									<?if($value == "char"):?>
											<?/*if($i == 1):?>
