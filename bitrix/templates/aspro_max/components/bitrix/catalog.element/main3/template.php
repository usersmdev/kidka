							...
							<div class="product-info-headnote__inner">
								...
								<?if($isArticle):?>
									<div class="product-info-headnote__article">
										<div class="article muted font_md" itemprop="additionalProperty" itemscope itemtype="http://schema.org/PropertyValue" <?if($arResult['SHOW_OFFERS_PROPS']){?>id="<? echo $arItemIDs["ALL_ITEM_IDS"]['DISPLAY_PROP_ARTICLE_DIV'] ?>" style="display: none;"<?}?>>
								...
								<div class="product-info-headnote__rating">
									<?$frame = $this->createFrame('dv_'.$arResult["ID"])->begin();?>
										<!-- <div class="rating_"> -->
											...
											<div class="ya-share2" data-services="collections,vkontakte,facebook,odnoklassniki,moimir,twitter,telegram" data-size="s"></div> -->
											...
											<!-- </div> -->
										<!-- </div> -->
										<?/*<div class="share" style="width: 200px;height:20px">
					...
					<?if($bShowPropsBlock):?>
						<div class="product-chars flex-50">
							...
							<?if( ($arResult['DISPLAY_PROPERTIES'] || $arResult['DISPLAY_PROPERTIES_OFFERS']) && $arParams['VISIBLE_PROP_COUNT'] > 0 ):?>
								...
								<?
								$arSmallTabOrder[] = 'char';
								$smallPropsContent = ob_get_clean();
								?>
							<?endif;?>
							<?
							$arSmallTabOrder[] = 'instruction';
							$arSmallTabOrder[] = '3d_model';
							?>
							<div class="tabs tabs_compact">
								<ul class="nav nav-tabs font_upper_md"><?
									?><?$iTab = 0;?><?
									?><?foreach($arSmallTabOrder as $value):?><?
										?><?//show char block?><?
										?><?if($value == "char"):?><?
											?><li class="bordered rounded3 <?=(!($iTab++) ? 'active' : '')?>"><a href="#props_short" data-toggle="tab"><?= Loc::getMessage("T_SMALL_CHARACTERISTICS") ?></a></li><?
										?><?endif;?><?
										?><?//show howbuy block?><?
										?><?if($value == "instruction"):?><?
											?><li class="bordered rounded3 <?=(!($iTab++) ? 'active' : '')?>"><a href="#instruction" data-toggle="tab"><?= Loc::getMessage("T_SMALL_INSTRUCTION") ?></a></li><?
										?><?endif;?><?
										?><?if($value == "3d_model"):?><?
											?><li class="bordered rounded3 <?=(!($iTab++) ? 'active' : '')?>"><a href="#3d_model" data-toggle="tab"><?= Loc::getMessage("T_SMALL_3D_MODEL") ?></a></li><?
										?><?endif;?><?
									?><?endforeach;?>
								</ul>
							</div>
							<div class="tab-content<?//=(count($arSmallTabOrder) <= 1 ? ' not_tabs' : '')?>">
								<?$iTab = 0;?>
								<?foreach($arSmallTabOrder as $value):?>
									<?//props?>
									<?if($value == "char"):?>
										<div class="tab-pane <?=$value;?> <?=(!($iTab++) ? 'active' : '')?>" id="props_short">
											<?/*if($i == 1):?>
									...
									<?//how buy?>
									<?if($value == "instruction"):?>
										<div class="tab-pane <?=$value;?> <?=(!($iTab++) ? 'active' : '')?>" id="instruction">
											<?/*if($i == 1):?>
												<div class="ordered-block__title option-font-bold font_lg">
									...
									<?endif;?>
									<?if($value == "instruction"):?>
										<div class="tab-pane <?=$value;?> <?=(!($iTab++) ? 'active' : '')?>" id="3d_model">
											<?/*if($i == 1):?>
												<div class="ordered-block__title option-font-bold font_lg">
													<?=$arParams["TITLE_HOW_BUY"];?>
												</div>
