...
<?$bShowLeftBlock = ($APPLICATION->GetProperty("HIDE_LEFT_BLOCK") != "Y" && !($arTheme['HEADER_TYPE']['VALUE'] == 28 || $arTheme['HEADER_TYPE']['VALUE'] == 29));?>
	...
	<div class="section-content-wrapper <?=($bShowLeftBlock ? 'with-leftblock' : '');?>">
		...
		if(true)
		{
			...
			if($arParams['HIDE_NOT_AVAILABLE'] == 'Y')
				$catalog_available = 'Y';
			// $arElementFilter = array("SECTION_ID" => $arSection["ID"], "ACTIVE" => "Y", "INCLUDE_SUBSECTIONS" => $arParams["INCLUDE_SUBSECTIONS"], "IBLOCK_ID" => $arParams["IBLOCK_ID"], "ELEMENT_IBLOCK_ID" => $arParams['ELEMENT_IBLOCK_ID']);
			$arElementFilter = ["ACTIVE" => "Y", "IBLOCK_ID" => $arParams["ELEMENT_IBLOCK_ID"]];
			if (Loader::includeModule('catalog')) {
				$basePrice = CCatalogGroup::GetBaseGroup();
				$arElementFilter['!CATALOG_PRICE_'.$basePrice['ID']] = false;
			}
			...
			if (Loader::includeModule('rumaster.utils') && \Rumaster\Utils\Helpers\StringHelper::endsWith($arParams['SEF_FOLDER'], '/top/')) {
				if (empty($GLOBALS[$arParams['FILTER_NAME']])) {
					$GLOBALS[$arParams['FILTER_NAME']] = [];
				}
				$arElementFilter['PROPERTY_TOP_TOVAROV'] = '7620';
				$GLOBALS[$arParams['FILTER_NAME']]['PROPERTY_TOP_TOVAROV'] = '7620';
			}
			if (Loader::includeModule('rumaster.utils') && \Rumaster\Utils\Helpers\StringHelper::endsWith($arParams['SEF_FOLDER'], '/sale/')) {
				if (empty($GLOBALS[$arParams['FILTER_NAME']])) {
					$GLOBALS[$arParams['FILTER_NAME']] = [];
				}
				$arElementFilter['PROPERTY_RASPRODAZHA'] = '7614';
				$GLOBALS[$arParams['FILTER_NAME']]['PROPERTY_RASPRODAZHA'] = '7614';
			}
			if (Loader::includeModule('rumaster.utils') && \Rumaster\Utils\Helpers\StringHelper::endsWith($arParams['SEF_FOLDER'], '/latest/')) {
				if (empty($GLOBALS[$arParams['FILTER_NAME']])) {
					$GLOBALS[$arParams['FILTER_NAME']] = [];
				}
				$arElementFilter['PROPERTY_NOVINKA'] = '7616';
				$GLOBALS[$arParams['FILTER_NAME']]['PROPERTY_NOVINKA'] = '7616';
			}
			$itemsCnt = CMaxCache::CIBlockElement_GetList(array("CACHE" => array("TAG" => CMaxCache::GetIBlockCacheTag($arParams["ELEMENT_IBLOCK_ID"]))), CMax::makeElementFilterInRegion($arElementFilter), array());
			...
			if(!$typeTmpSKU || !$viewTmpFilter){
				if($section['DEPTH_LEVEL'] > 1){
					$sectionParent = CMaxCache::CIBlockSection_GetList(array('CACHE' => array("MULTI" =>"N", "TAG" => CMaxCache::GetIBlockCacheTag($arParams["IBLOCK_ID"]))), array('GLOBAL_ACTIVE' => 'Y', "ID" => $section["IBLOCK_SECTION_ID"], "IBLOCK_ID" => $arParams["IBLOCK_ID"]), false, array("ID", "IBLOCK_ID", "NAME", "UF_OFFERS_TYPE", 'UF_FILTER_VIEW'));
		...
		if($arRegion)
		{
			...
			$GLOBALS[$arParams['FILTER_NAME']]['IBLOCK_ID'] = $arParams['ELEMENT_IBLOCK_ID'];
			CMax::makeElementFilterInRegion($GLOBALS[$arParams['FILTER_NAME']]);
		}
		if (Loader::includeModule('catalog')) {
			$basePrice = CCatalogGroup::GetBaseGroup();
			$GLOBALS[$arParams['FILTER_NAME']]['!CATALOG_PRICE_'.$basePrice['ID']] = false;
		}
