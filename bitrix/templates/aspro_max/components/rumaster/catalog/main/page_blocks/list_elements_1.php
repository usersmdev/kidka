...
<?if($itemsCnt):?>
			...
			<?}?>
				...
				<div class="smart-filter-filter" style="display: none;"><?if($SMART_FILTER_FILTER) {print_r(\Bitrix\Main\Web\Json::encode($SMART_FILTER_FILTER));}?></div>

				<?if($SMART_FILTER_SORT):?>
					<div class="smart-filter-sort" style="display: none;"><?=\Bitrix\Main\Web\Json::encode($SMART_FILTER_SORT)?></div>
				<?endif;?>
				<?
				CatalogHelper::fixCatalogFilter($arParams["FILTER_NAME"], $arResult["VARIABLES"]["SECTION_ID"]);
				?>
				<?$APPLICATION->IncludeComponent(
					...
					Array(
						...
						"IBLOCK_TYPE" => $arParams["IBLOCK_TYPE"],
						"IBLOCK_ID" => $arParams["ELEMENT_IBLOCK_ID"],
