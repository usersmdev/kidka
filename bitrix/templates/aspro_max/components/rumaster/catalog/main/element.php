...
if($arResult["VARIABLES"]["ELEMENT_ID"] > 0)
	$arElementFilter = array("IBLOCK_ID" => $arParams["ELEMENT_IBLOCK_ID"], "ID" => $arResult["VARIABLES"]["ELEMENT_ID"]);
elseif(strlen(trim($arResult["VARIABLES"]["ELEMENT_CODE"])) > 0)
	$arElementFilter = array("IBLOCK_ID" => $arParams["ELEMENT_IBLOCK_ID"], "=CODE" => $arResult["VARIABLES"]["ELEMENT_CODE"]);
...
$APPLICATION->SetPageProperty("body_class", "page-products-element");
if($arParams['STORES'])
{
...
if(!$typeTmpSKU || $sectionTizers || !$sectionHelpText){
	if($arSection["DEPTH_LEVEL"] > 2){
		$arSectionParent = CMaxCache::CIBlockSection_GetList(array('CACHE' => array("MULTI" =>"N", "TAG" => CMaxCache::GetIBlockCacheTag($arParams["IBLOCK_ID"]))), array('GLOBAL_ACTIVE' => 'Y', "ID" => $arSection["IBLOCK_SECTION_ID"], "IBLOCK_ID" => $arParams["IBLOCK_ID"]), false, array("ID", "IBLOCK_ID", "UF_OFFERS_TYPE", "UF_SECTION_TIZERS", "UF_HELP_TEXT"));
